...
const [commentStatus,setCommentStatus]=useState('')
  ...
  async function addComment(commentObj) {
     ///addd name of user to comment obj
        ...
        {/* edit or delete */}
        {
          currentUser.role==='author' && (
            <div className="d-flex me-3">
               <button className="me-2 btn btn-light small-btn edit-btn" onClick={enableEdit}>
                <FaEdit className="text-warning small-icon " />
               </button>
               {/* ifarticle is active, display delete icon,otherwise display restore icon */}
               {
                state. isArticleActive===true?(
                  <button className="me-2 btn btn-light small-btn" onClick={deleteArticle}>
                      <MdDelete className="text-danger fs-4 small-icon"/>
                  </button>
                ):(
                  <button className="me-2 btn btn-light small-btn" onClick={restoreArticle}>
                      <MdRestore className="text-info fs-4 small-icon"/>
                  </button>
                )
               }
            </div>
         ...
         {/* edit form */}
          <form onSubmit={handleSubmit(onSave)}>
            <div className="mb-4">
             ...
             <input type="text" className="form-control" {...register("title")} defaultValue={state.title} id="title" />
            </div>
            <div className="mb-4">
              <label htmlFor="category" className="form-label">Select a category</label>
              <select className="form-select" {...register("category")} id="category" defaultValue={state.category}>
